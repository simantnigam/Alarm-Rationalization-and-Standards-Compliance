...
closed probe) and real FastEmbed models; nothing mocked.
"""
...
def test_stale_alarm_query_retrieves_clause_2_2(
    qdrant_client_with_one_document: QdrantClient, embedder: Embedder
) -> None:
    service = RetrievalService(
        qdrant_client_with_one_document, embedder, collection_name=COLLECTION
    )
    citations = service.retrieve("What counts as a stale alarm?")
...
def test_citation_renders_in_the_documented_format(
    ...
    citations = service.retrieve("stale alarm definition")
    top = citations[0]
...
def test_flood_query_retrieves_clause_2_6(
    ...
    citations = service.retrieve("How many alarms in 10 minutes counts as a flood?")
    assert any(c.clause_id == "2.6" for c in citations)
